...
from app.models import worker, capture, detection, violation  # import semua model agar tabel terbuat
from app.database import Base
from app.routers import workers, captures, detections, violations

# Buat semua tabel di database saat aplikasi pertama kali dijalankan
# Jika tabel sudah ada, ini tidak akan menimpa data yang ada
...
3. **Deteksi AI** - Modul AI menganalisis foto dan mengirim hasilnya ke `POST /api/detections/`
4. **Input Manual** - Pengawas K3 melihat hasil AI dan mencatat pelanggar via `POST /api/violations/`
...
# CORS Middleware - mengizinkan frontend (dashboard web) mengakses API ini
# Sesuaikan origins jika dashboard sudah punya domain/port tertentu
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Daftarkan semua router (kelompok endpoint)
